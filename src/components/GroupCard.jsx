...
function GroupCard({ group, sectionId, index, onEditItem }) {
  const { deleteItem, addMainItem, expandedGroups, toggleGroup } = useNavigation();
  ...
  const handleNameBlur = () => {
    if (groupName.trim()) {
      updateItem(group.id, { label: groupName.trim() });
    } else {
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: isExpanded ? '8px' : '0' }}>
        ...
        <button
          onClick={() => addMainItem(sectionId, group.id)}
          style={{
            padding: '4px 8px',
            fontSize: '12px',
